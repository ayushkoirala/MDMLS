...
def create_dataloader(df, tokenizer, batch_size):
    df.dropna(subset=['input'])
    df = df[df['input'].apply(lambda x: isinstance(x, str))]
    inputs = tokenizer(list(df['input']), padding="max_length", truncation=True, return_tensors="pt")
...
def get_and_save_top_k_sentences_with_token_length(val_df, k_values, tokenizer,result_path):
    for k in k_values:
        ...
        for name, group in val_df.groupby('ReviewID'):
            ...
            first_target = group['Target'].iloc[0]
...
def calculate_rouge_for_each_review(df):
    ...
    for review_id in unique_review_ids:
        ...
        target_aggregated = filtered_df['Target'].iloc[0]
...
def main():
    ...
    display(f"{args=}")
    val_path = r"/home/akoirala/Thesis/training_query_model/PICO_0.15_validation.csv"
    checkpoint_path = r"/home/akoirala/Thesis/training_query_model/checkpoint/checkpoint-3"
    result_path = r"/home/akoirala/Thesis/training_query_model/result_demo"
    ...
    display(f"Imported {len(val_df)} rows of train datasset")

    tokenizer, model = init_model_and_tokenizer(checkpoint_path)
    display(f"tokenizer and model loded")

    dataloader = create_dataloader(val_df, tokenizer,batch_size=args.bs)
    ...
    get_and_save_top_k_sentences_with_token_length(val_df, [10, 15, 20, 25, 30], tokenizer,result_path)
...
# Load these into DataFrames.
    ...
    plot_and_save_boxplot(dfs, save_path='my_custom_boxplot.png')

    rouge_score = calculate_rouge_for_each_review(val_df)
    display(f"average rouge score of Extractive summary")
    for k, df in zip([10, 15, 20, 25, 30], dfs):
