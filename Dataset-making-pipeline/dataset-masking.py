...
# def randomly_mask_text(text, ratio):
#     words = text.split()
...
def randomly_mask_text(text, ratio, priority_words=None):
...
def preprocess_and_save(input_filepath, output_dir, tokenizer, model, device, mask_ratio):
    # Load dataframe
    ...
    df['Masked Background'] = df['Background'].apply(randomly_mask_text, args=(mask_ratio,))
...
def main():
    ...
    parser.add_argument('--result', type=str, default='/home/akoirala/Thesis/Data-making-pipeline/random_framework_final', help='Path to the result dataset')
    ...
    for ratio in ratios:
        ...
        for filename in files:
            ...
            preprocess_and_save(input_filepath, output_directory, tokenizer, model, device, ratio)
