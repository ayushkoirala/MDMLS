...
def quantize_tensor(tensor, num_bits=4):
    ...
    This is a simple example and might not be optimal.
    """
    qmin = 0.
    qmax = 2.**num_bits - 1.
    ...
    q_tensor = (tensor / scale + 0.5).clamp(qmin, qmax).floor()  # Simple rounding and clamping
    q_tensor = q_tensor * scale - 0.5 * (qmax - qmin) * scale  # Dequantize to original scale
    
    return q_tensor

def qlora_adaptation(model, rank):
    """
    Adapts a model using QLORA by adding a low-rank matrix to its parameters 
    and quantizing the original parameters to 4-bits.
    
    Returns the low-rank parameters and the model.
    """
    lora_params = []
    for name, param in list(model.named_parameters()):
        lora_name = name.replace('.', '_') + "_lora"
        lora_param = torch.nn.Parameter(torch.zeros_like(param).requires_grad_())
        model.register_parameter(lora_name, lora_param)
        param.data += lora_param.data
        
        # Quantize the original parameter to 4-bits
        param.data = quantize_tensor(param.data, num_bits=4)
        param.requires_grad = True  # If you want to fine-tune original weights; set False otherwise
        lora_params.append(lora_param)

    return lora_params, model
# def lora_adaptation(model, rank):
#     """
#     Adapts a model using LORA by adding a low-rank matrix to its parameters.
...
#         lora_param = torch.zeros_like(param).requires_grad_()
#         param.requires_grad = False
#         lora_params.append(lora_param)
#         param.data += lora_param
#     return lora_params, model
...
#         self.data = data
#         combined_sentences = ['<Background>' + bg.replace("[MASK]", "<mask>") + '</Background><sentences>' + sentence + '</sentences>' for bg, sentence in zip(data['Masked Background'], data['top_k_sentences'])]
...
def build_loader(data, split, tokenizer, bs):
...
def train(args, train_data, val_data):
    ...
    #special_tokens_dict = {'additional_special_tokens': ['<Background>', '</Background>', '<sentences>', '</sentences>']}
    #tokenizer.add_special_tokens(special_tokens_dict)
    model = BartForConditionalGeneration.from_pretrained(MODEL_PATH[args.model])
    #model.resize_token_embeddings(len(tokenizer))
    rank = 10  # or whatever rank you choose
    lora_params, model = qlora_adaptation(model, rank)
    for param in lora_params:
        print(".............",param.requires_grad)
    train_loader = build_loader(train_data, split='train', tokenizer=tokenizer, bs=args.bs)
    ...
    model.to(device)
    train_records = recorder(main_folder="random_0.15_masked/generator_records", key_dir=args.model)
    model_saving = save_model(model_name=args.model)
    best_score = 0
...
#     device = torch.device(f'cuda' if torch.cuda.is_available() else 'cpu')
#     model.to(device)
#     train_records = recorder(main_folder="generator_records", key_dir=args.model)
#     model_saving = save_model(model_name=args.model)
#     best_score = 0
#     for epoch in range(args.n_epoch):
#         train_losses = []
...
#             optimizer.step()
#             train_losses.append(loss.item())
#             if iter==0: display_cuda_info(short=True, isPrint=False)
#         train_records.record('train_losses', train_losses)
#         train_records.save()
#         with torch.no_grad():
#             gen_seq_list, ref_seq_list = [], []
...
#                 ref_seq_list.extend([tokenizer.decode(ids, skip_special_tokens=True) for ids in batch['labels']])
#             val_scores = cal_matrix(pred=gen_seq_list, ref=ref_seq_list, device=device)
#             for key, val in val_scores.items(): train_records.record(key, val)
#             if val_scores['rouge1']>best_score:
#                 model_saving.save(model)
#                 display(f"[End Epoch {epoch+1}] Reach new highest val rouge-1 ({val_scores['rouge1']*100:.2f})")
#                 best_score = val_scores['rouge1']
...
class save_model:
    def __init__(self,model_name, main_folder="random_0.15_masked/generator_checkpoints", subfolder='checkpoint', train_id=None):
...
class recorder:
    def __init__(self, main_folder="records", key_dir="default"):
        self.save_dir = os.path.join(main_folder, key_dir)
        self.record_dict = {}
